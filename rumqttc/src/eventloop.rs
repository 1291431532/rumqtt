...
impl EventLoop {
    ...
    async fn select(&mut self) -> Result<Event, ConnectionError> {
        ...
        let future1 = network.readb(&mut self.state);
        let future2 = self.requests_rx.recv();
        let future3 = next_pending(throttle, &mut self.pending);
        let future4 = self.keepalive_timeout.as_mut().unwrap();
        let future5 = self.cancel_rx.recv();
        let select5 = Select5 {
            future1: Some(future1),
            future2: if !inflight_full && !pending && !collision { Some(future2) } else { None },
            future3: if pending { Some(future3) } else { None },
            future4: Some(future4),
            future5: Some(future5),
