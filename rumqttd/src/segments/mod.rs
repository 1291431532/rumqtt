...
use memory::segment::MemorySegment;
...
pub struct CommitLog {
    ...
    tail: u64,
    /// Current number of segments on disk
    disk_segments_count: usize,
...
impl CommitLog {
    ...
    pub fn new(
        ...
        match disk {
            Some((path, max_disk_segments)) => {
                let mut segments = VecDeque::with_capacity(max_disk_segments + max_mem_segments);

                // adding pre-existing disks segmnets, if any
                let (head, tail, absolute_offset, disk_handler) =
                    DiskHandler::new(path, &mut segments)?;
                // adding 1 in memory segment
                ...
                Ok(Self {
                    head,
                    tail,
                    disk_segments_count: (tail - head) as usize,
                    max_segment_size,
                    max_mem_segments,
                    max_disk_segments,
                    segments,
    ...
    #[inline]
    pub fn disk_segments_count(&self) -> usize {
        self.disk_segments_count
    }

    #[inline]
    pub fn memory_segments_count(&self) -> usize {
        self.segments.len() - self.disk_segments_count
    }
    ...
    fn active_segment_mut(&mut self) -> &mut MemorySegment {
        match self.segments.back_mut().unwrap().inner {
            ...
            // SAFETY: last element always a memory segment, we don't allow active segment to be
            // on-disk.
    ...
    pub fn append(&mut self, bytes: Bytes) -> (u64, u64) {
        ...
        debug!("appending at ({}, {})", self.tail, absolute_offset);
    ...
    fn apply_retention(&mut self) {
        if self.active_segment().size() >= self.max_segment_size as u64 {
            ...
            if self.memory_segments_count() >= self.max_mem_segments {
                ...
                if self.disk_handler.is_some() {
                    self.apply_disk_retention()
                } else {
    ...
    fn apply_disk_retention(&mut self) {
        let handler = self.disk_handler.as_mut().unwrap();
        if self.disk_segments_count >= self.max_disk_segments {
            // doing pop_front here because if file handles to disk still exist then
            // they might flush back the data onto the disk.
            self.segments.pop_front();
            self.disk_segments_count -= 1;
            if let Err(e) = handler.delete(self.head) {
                warn!("unable to delete file at index {}: {:?}", self.head, e);
            }

            self.head += 1;
        }

        let segment_id = self.head + self.disk_segments_count as u64;
        let inmemory_head = &mut self.segments[self.disk_segments_count];
        match inmemory_head.move_to_disk(&handler.dir, segment_id, &mut handler.hasher) {
            Ok(..) => {
                // only increase disk_segments_count when actually saving on disk
                self.disk_segments_count += 1;
            }
            Err(e) => {
                warn!("unable to write segment {} to disk: {:?}", segment_id, e);
            }
        }
    }
...
mod test {
    ...
    use super::disk::{CommitFile, InvalidType};
    ...
    fn reading_at_invalid_cursor_returns_none() {
        // 1 as active only
        let log = CommitLog::new(1024, 1, None).unwrap();
        let mut out = Vec::new();
    ...
    fn inmemory_appends_and_retention_policy_works() {
        ...
        let mut log = CommitLog::new(max_segment_size, 2, None).unwrap();
    ...
    fn active_segment_appends_and_reads_works() {
        ...
        // 1 as active only
        let mut log = CommitLog::new(max_segment_size, 1, None).unwrap();
    ...
    fn read_switch_from_active_to_inactive_to_active_segment_works() {
        ...
        let mut log = CommitLog::new(max_segment_size, 4, None).unwrap();
    ...
    fn read_with_jumps_works() {
        ...
        let mut log = CommitLog::new(max_segment_size, 5, None).unwrap();
    ...
    fn read_jump_from_deleted_segment_works() {
        ...
        let mut log = CommitLog::new(max_segment_size, 10, None).unwrap();
    ...
    #[test]
    fn ondisk_appends_and_retention_policy_works() {
        let max_segment_size = 1024 * 100; // 100K
        let packet_size = 1024;
        let dir = tempdir().unwrap();

        // 1 as active 1 as inactive in mem 1 on disk
        let mut log = CommitLog::new(max_segment_size, 2, Some((dir.path().into(), 1))).unwrap();
        ...
        // Append more data to trigger saving onto disk
        log.append(random_payload(200, packet_size));
        assert_eq!(log.head, 0);
        assert_eq!(log.tail, 2);
        assert_eq!(log.disk_segments_count, 1);
        assert_eq!(log.len(), 3);

        // Fill the rest of new active segment
        for (i, v) in (201..300).enumerate() {
            let offset = log.append(random_payload(v as u8, packet_size));
            assert_eq!(offset, (2, i as u64 + 202))
        }
        assert_eq!(log.head, 0);
        assert_eq!(log.tail, 2);
        assert_eq!(log.disk_segments_count, 1);
        ...
        // Append more data to trigger new segment creation and retention policy
        log.append(random_payload(100 as u8, packet_size));
        assert_eq!(log.head, 1);
    ...
    #[test]
    fn reading_from_disk_works_fine() {
        let max_segment_size = 1024 * 10; // 10K
        let packet_size = 1024;
        let dir = tempdir().unwrap();

        // 1 as active 1 as inactive in mem 10 on disk
        let mut log = CommitLog::new(max_segment_size, 2, Some((dir.path().into(), 10))).unwrap();

        // Fill a total of 12 segments (120 * 1K = 1_20K = 12 segments), each having 10 payloads.
        for i in 0..120 {
            log.append(random_payload(i, packet_size));
        }
        assert_eq!(log.disk_segments_count(), 10);
        assert_eq!(log.len(), 12);

        // Read all the disk segments to verify index and segment files are in sync. jumping logic
        // is tested in previous tests as it is independent of disk/mem.

        // Reading in middle
        let mut out = Vec::new();
        let next = log.readv((2, 25), 50, &mut out).unwrap();
        assert_eq!(
            next,
            Next {
                start: (2, 25),
        ...
        out.into_iter()
            .enumerate()
            .for_each(|(i, v)| verify(i + 25, packet_size, v));

        // Reading from start to end in 1 go
        let mut out = Vec::new();
        let next = log.readv((0, 0), 100, &mut out).unwrap();
        assert_eq!(
            next,
            Next {
                start: (0, 0),
        ...
        // Reading from disk and mem
        let mut out = Vec::new();
        let next = log.readv((1, 19), 100, &mut out).unwrap();
        assert_eq!(
            next,
            Next {
                start: (1, 19),
        ...
        out.into_iter()
            .enumerate()
            .for_each(|(i, v)| verify(i + 19, packet_size, v));

        // Reading beyond size of segment, should jump to next
        let mut out = Vec::new();
        let next = log.readv((2, 30), 20, &mut out).unwrap();
        assert_eq!(
            next,
            Next {
                start: (2, 30),
        ...
        out.into_iter()
            .enumerate()
            .for_each(|(i, v)| verify(i + 30, packet_size, v));
    }

    #[test]
    fn recovers_from_disk_after_closing_log() {
        let max_segment_size = 1024 * 10; // 10K
        let packet_size = 1024;
        let dir = tempdir().unwrap();

        // 1 as active 1 as inactive in mem 10 on disk
        let mut log = CommitLog::new(max_segment_size, 1, Some((dir.path().into(), 10))).unwrap();

        // Fill a total of 6 segments, each having 10 payloads, resulting in 5 on disk.
        for i in 0..60 {
            log.append(random_payload(i, packet_size));
        }

        assert_eq!(log.disk_segments_count, 5);
        assert_eq!(log.segments.len(), 6);

        // 1 as active 1 as inactive in mem 10 on disk
        let mut log = CommitLog::new(max_segment_size, 1, Some((dir.path().into(), 10))).unwrap();
        assert_eq!(log.disk_segments_count, 5);
        // len is 6 not 5 as there is one implictly added active segment
        assert_eq!(log.segments.len(), 6);
        ...
        // Fill a total of 6 segments, each having 10 payloads, resulting in 5 new on disk, and a
        // total of 10 on disk.
        for i in 0..60 {
            log.append(random_payload(i + 50, packet_size));
        }

        assert_eq!(log.disk_segments_count, 10);
        assert_eq!(log.segments.len(), 11);

        // 1 as active 1 as inactive in mem 10 on disk
        let log = CommitLog::new(max_segment_size, 2, Some((dir.path().into(), 10))).unwrap();
        assert_eq!(log.disk_segments_count, 10);
        assert_eq!(log.segments.len(), 11);
    ...
    #[test]
    fn detects_missing_malicious_corrupt_files() {
        let max_segment_size = 1024 * 10; // 10K
        let packet_size = 1024;
        let dir = tempdir().unwrap();

        // 1 as active 1 as inactive in mem 10 on disk
        let mut log = CommitLog::new(max_segment_size, 1, Some((dir.path().into(), 20))).unwrap();

        // Fill a total of 21 segments, each having 10 payloads, resulting in 20 on disk.
        for i in 0..210 {
            log.append(random_payload(i, packet_size));
        }
        ...
        // delete some index and segment files
        remove_file(dir.path().join(format!("{:020}.index", 1))).unwrap();
        remove_file(dir.path().join(format!("{:020}.segment", 2))).unwrap();

        // change some file names
        rename(
            dir.path().join(format!("{:020}.index", 4)),
            dir.path().join("some_malicious_file_1"),
        )
        .unwrap();
        rename(
            dir.path().join(format!("{:020}.segment", 4)),
            dir.path().join("some_malicious_file_2"),
        )
        .unwrap();

        // change index file contents, adding and removing entries
        let index_file = OpenOptions::new()
            .write(true)
            .open(dir.path().join(format!("{:020}.index", 8)))
            .unwrap();
        let len = index_file.metadata().unwrap().len();
        index_file.set_len(len / 2).unwrap();
        let index_file = OpenOptions::new()
            .read(true)
            .write(true)
            .open(dir.path().join(format!("{:020}.index", 9)))
            .unwrap();
        let len = index_file.metadata().unwrap().len();
        index_file
            .write_at(b"abcdefghijklmnopqrstuvwxyz", len - 1)
            .unwrap();

        // changing the hash
        let index_file = OpenOptions::new()
            .write(true)
            .open(dir.path().join(format!("{:020}.index", 10)))
            .unwrap();
        index_file
            .write_at(b"hahahahahahahahahahahahahahahaha", 0)
            .unwrap();

        // corrupting segment files
        let segment_file = OpenOptions::new()
            .write(true)
            .open(dir.path().join(format!("{:020}.segment", 11)))
            .unwrap();
        segment_file.write_at(b"changing stuff", 42).unwrap();

        let log = CommitLog::new(max_segment_size, 1, Some((dir.path().into(), 20))).unwrap();

        __test_match!(
            SegmentType::Disk(CommitFile::Invalid(InvalidType::NoIndex)),
            log.segments[1].inner
        );
        __test_match!(
            SegmentType::Disk(CommitFile::Invalid(InvalidType::NoSegment)),
            log.segments[2].inner
        );
        __test_match!(
            SegmentType::Disk(CommitFile::Invalid(InvalidType::NoExist)),
            log.segments[4].inner
        );
        __test_match!(
            SegmentType::Disk(CommitFile::Invalid(InvalidType::InvalidChecksum)),
            log.segments[8].inner
        );
        __test_match!(
            SegmentType::Disk(CommitFile::Invalid(InvalidType::InvalidChecksum)),
            log.segments[9].inner
        );
        __test_match!(
            SegmentType::Disk(CommitFile::Invalid(InvalidType::InvalidChecksum)),
            log.segments[10].inner
        );
        __test_match!(
            SegmentType::Disk(CommitFile::Invalid(InvalidType::InvalidChecksum)),
            log.segments[11].inner
        );

        assert_eq!(log.segments[0].absolute_offset, 0);
        assert_eq!(log.segments[0].next_absolute_offset(), 10);
        assert_eq!(log.segments[1].absolute_offset, 10);
        assert_eq!(log.segments[1].next_absolute_offset(), 10);
        assert_eq!(log.segments[2].absolute_offset, 10);
        assert_eq!(log.segments[2].next_absolute_offset(), 10);
        assert_eq!(log.segments[3].absolute_offset, 10);
        assert_eq!(log.segments[3].next_absolute_offset(), 20);
        assert_eq!(log.segments[4].absolute_offset, 20);
        assert_eq!(log.segments[4].next_absolute_offset(), 20);
        assert_eq!(log.segments[5].absolute_offset, 20);
        assert_eq!(log.segments[5].next_absolute_offset(), 30);
        assert_eq!(log.segments[6].absolute_offset, 30);
        assert_eq!(log.segments[6].next_absolute_offset(), 40);
        assert_eq!(log.segments[7].absolute_offset, 40);
        assert_eq!(log.segments[7].next_absolute_offset(), 50);
        assert_eq!(log.segments[8].absolute_offset, 50);
        assert_eq!(log.segments[8].next_absolute_offset(), 50);
        assert_eq!(log.segments[9].absolute_offset, 50);
        assert_eq!(log.segments[9].next_absolute_offset(), 50);
        assert_eq!(log.segments[10].absolute_offset, 50);
        assert_eq!(log.segments[10].next_absolute_offset(), 50);
        assert_eq!(log.segments[11].absolute_offset, 50);
        assert_eq!(log.segments[11].next_absolute_offset(), 50);
        ...
        let mut out = Vec::new();
        let next = log.readv((1, 10), 10, &mut out).unwrap();
        assert_eq!(out.len(), 10);
        assert_eq!(
            next,
            Next {
        ...
        out.into_iter()
            .enumerate()
            .for_each(|(i, v)| verify(30 + i, packet_size, v));
    }
}
